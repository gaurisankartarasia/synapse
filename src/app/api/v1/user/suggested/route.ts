// //app/api/v1/user/suggested/route.ts
...
export async function GET() {
  try {
    ...
    // Get current user's blocked users
    const blockedUsersSnapshot = await db.collection("users").doc(currentUid).collection("blocked").get();
    const blockedUids = blockedUsersSnapshot.docs.map((doc) => doc.id);

    // Get current user's following users
    const followingSnapshot = await db.collection("users").doc(currentUid).collection("following").get();
    const followingUids = followingSnapshot.docs.map((doc) => doc.id);

    // Get random users excluding current user, blocked users, and followed users
    const usersSnapshot = await db
      .collection("users")
      .where("uid", "!=", currentUid)
      .limit(10) // Increased limit to ensure we get 5 after filtering
      .get();

    const users = await Promise.all(
      usersSnapshot.docs
        .filter((doc) => !blockedUids.includes(doc.id) && !followingUids.includes(doc.id))
        .slice(0, 5)
        .map(async (doc) => {
          const userData = doc.data();
          const followRequestStatus = userData.isPrivate
          ...
          return {
            ...
            isVerified: userData.isVerified,
            isPrivate: userData.isPrivate,
            isFollowing: false, // Since we're filtering out followed users
            isRequested: followRequestStatus?.exists || false,
          };
        })
    );
