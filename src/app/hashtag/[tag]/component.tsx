...
const HashtagPage = () => {
  ...
  const fetchHashtagPosts = useCallback(async (lastId: string | null = null) => {
    ...
    try {
      ...
      const response = await axiosInstance.get(`/hashtag/${encodedTag}`, {
        params: lastId ? { lastPostId: lastId } : {},
      });
      ...
      if (axios.isAxiosError(error)) {
        console.error("Axios error fetching hashtag posts:", error.response?.data || error.message);
        setError(error.response?.data?.error || "Failed to fetch hashtag posts");
      } else {
  ...
  // Add tag as a dependency since it's used in the function
