    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button 
          variant="ghost" 
          className={cn(
            isMobile ? "h-10 w-10" : "w-full flex items-center justify-center lg:justify-start gap-4 py-4 lg:py-6 px-3 hover:bg-accent/50 rounded-md transition-all"
          )}
        >
          < Menu />
          {!isMobile && (
      ...
      <aside className="hidden md:block fixed left-0 top-0 h-screen w-20 lg:w-80 border-r bg-background/95 transition-all duration-300 shadow-sm">
        <div className="flex flex-col h-full p-4">
          ...
          <nav className="flex flex-1 flex-col gap-4">
            ...
            {/* Add profile link to the navigation */}
            <NavigationLink
