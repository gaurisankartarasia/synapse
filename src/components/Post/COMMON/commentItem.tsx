...
export const CommentItem = ({
  ...
  const [replyReportData, setReplyReportData] = useState<{
    commentId: string;
    replyId: string;
  } | null>(null);
  ...
  const handleReportSubmit = async (reason: string) => {
    if (replyReportData?.replyId) {
      await onReport(
        replyReportData.commentId,
        reason,
        replyReportData.replyId
      );
    } else {
      ...
      <div className="flex justify-between items-start">
        <div className="w-full">
          <div className="flex items-center gap-1">
            ...
            <span>
              {comment.user.isVerified && <Verified fontSize="small" />}
            </span>
          </div>
              ...
              {comment.replies?.map((reply) => (
                <div key={reply.id} className="border-l pl-4 py-2">
                  <div className="flex items-center gap-1">
                    ...
                    <span>
                      {reply.user.isVerified && <Verified fontSize="small" />}
                    </span>
                  </div>
